...
public record DepTypeTerm(@NotNull DTKind kind, @NotNull Term param,
  ...
  public @NotNull Term coe(@NotNull LocalVar i, CoeTerm coe) {
    return switch (kind) {
      case Pi -> coePi(i, coe);
      case Sigma -> coeSigma(i, coe);
    };
  }
  private @NotNull Term coeSigma(@NotNull LocalVar i, CoeTerm coe) {
    ...
    return new LamTerm(new Closure.Jit(p -> {
      ...
      // i : I |- A i
      var A = param().bind(i);
      Function<Closure, Closure> B = a -> abstractBia(i, a);
  ...
  private @NotNull Term coePi(@NotNull LocalVar i, CoeTerm coe) {
    // We are trying to construct a term for:
