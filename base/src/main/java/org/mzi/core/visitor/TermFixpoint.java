...
public interface TermFixpoint<P> extends
  ...
  private <T> T visitParameterized(
    @NotNull CoreParam theParam, @NotNull Term theBody, @NotNull P p, @NotNull T original,
    @NotNull BiFunction<@NotNull CoreParam, @NotNull Term, T> callback
  ) {
    var param = new CoreParam(theParam.ref(), theParam.type().accept(this, p), theParam.explicit());
  ...
  @Override default @NotNull Term visitSigma(@NotNull SigmaTerm term, P p) {
    var params = term.params().map(param ->
      new CoreParam(param.ref(), param.type().accept(this, p), param.explicit()));
