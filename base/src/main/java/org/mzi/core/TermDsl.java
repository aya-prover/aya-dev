...
public class TermDsl extends LispBaseVisitor<Term> {
  ...
  public static @NotNull ImmutableSeq<@NotNull CoreParam> parseTele(@NotNull String text, @NotNull Map<String, @NotNull Var> refs) {
  ...
  public @NotNull CoreParam exprToParam(LispParser.ExprContext ctx) {
    var atom = ctx.atom();
    if (atom != null) {
    ...
    return new CoreParam(ref(ident), exprs.get(0).accept(this), explicit);
  }

  public @NotNull ImmutableSeq<@NotNull CoreParam> exprToParams(LispParser.ExprContext ctx) {
    var atom = ctx.atom();
    if (atom != null) {
    ...
    assert exprs.size() == 3;
    boolean explicit = licit(exprs);
    return exprToParams(exprs.get(2)).prepended(new CoreParam(ref(ident), exprs.get(0).accept(this), explicit));
  }
