...
public interface StmtConsumer extends Consumer<Stmt>, EndoExpr, EndoPattern {
  default void accept(Stmt stmt) {
    switch (stmt) {
      case Remark remark -> {
      ...
      case Decl decl -> {
        ...
        switch (decl) {
          ...
          case TeleDecl.FnDecl fn -> fn.body = fn.body.map(this, clauses -> clauses.map(this::clause));
          case TeleDecl.DataCtor ctor -> {
            ctor.patterns = ctor.patterns.map(this::apply);
            ctor.clauses = ctor.clauses.map(this::clause);
          }
  ...
  default @NotNull Pattern.Clause clause(Pattern.@NotNull Clause c) {
    return new Pattern.Clause(c.sourcePos, c.patterns.map(this::apply), c.expr.map(this));
  }
}
