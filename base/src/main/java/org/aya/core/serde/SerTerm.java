...
public sealed interface SerTerm extends Serializable {
  record DeState(
    @NotNull MutableMap<Seq<String>, MutableMap<String, GenericDefVar<?, ?>>> defCache,
    ...
    public <V extends GenericDefVar>
    ...
    @SuppressWarnings("unchecked") <V extends GenericDefVar<?, ?>>
    @NotNull V newDef(@NotNull SerDef.QName name, @NotNull Function<String, V> factory) {
      // We assume this cast to be safe
      var defVar = factory.apply(name.name());
      var old = defCache
    ...
    @NotNull DefVar<Core, Concrete> newDef(@NotNull SerDef.QName name) {
      return this.newDef(name, DefVar::empty);
    }

    @SuppressWarnings("unchecked") <Core extends ClassDef, Concrete extends ClassDecl>
    @NotNull ClassDefVar<Core, Concrete> newClassDef(@NotNull SerDef.QName name) {
      return this.newDef(name, ClassDefVar::empty);
    }
