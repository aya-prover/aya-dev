...
public record RefTerm(@NotNull LocalVar var, @NotNull Term type) implements Term {
   ...
   */
  public static @NotNull Term simpleEta(@NotNull Term term) {
    return switch (term) {
  ...
  private static @NotNull Term etaLambda(@NotNull IntroTerm.Lambda lambdaTerm) {
    var etaBodyTerm = simpleEta(lambdaTerm.body());
    ...
    if (lastTerm instanceof RefTerm lastRefTerm
      ...
      && appearFree(lastRefTerm, bodyWithoutLastTerm)) return simpleEta(bodyWithoutLastTerm);
  ...
  private static @NotNull Term etaTuple(@NotNull IntroTerm.Tuple tupleTerm) {
    var etaItems = tupleTerm.items().map(RefTerm::simpleEta);
  ...
  private static @NotNull Term etaApp(@NotNull ElimTerm.App appTerm) {
    return new ElimTerm.App(appTerm.of(), new Arg<>(simpleEta(appTerm.arg().term()),
      appTerm.arg().explicit()));
  }

  private static @NotNull Term etaProj(@NotNull ElimTerm.Proj projTerm) {
    return new ElimTerm.Proj(simpleEta(projTerm.of()), projTerm.ix());
  }
}
