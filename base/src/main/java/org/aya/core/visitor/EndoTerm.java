  ...
  record Elevator(int lift) implements EndoTerm {
    ...
    @Override public @NotNull Term post(@NotNull Term term) {
      return switch (term) {
        ...
        case ReduceRule.Con con -> new ReduceRule.Con(con.rule(), con.ulift() + lift, con.dataArgs(), con.conArgs());
        case ReduceRule.Fn fn -> new ReduceRule.Fn(fn.rule(), fn.ulift() + lift, fn.args());
