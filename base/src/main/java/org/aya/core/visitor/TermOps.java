    ...
    private @Nullable WithPos<Term> unfoldClauses(
      ...
      for (var match : clauses) {
        ...
        if (result.isOk()) {
          ...
          var body = match.body().subst(subst).normalize(state, NormalizeMode.NF);
    ...
    @Override public @NotNull Term post(@NotNull Term term) {
      return switch (view.post(term)) {
        ...
        case CallTerm.Fn fn -> {
          ...
          yield def.body.fold(
            lamBody -> lamBody.subst(buildSubst(def.telescope(), fn.args())).normalize(state, NormalizeMode.NF),
            patBody -> {
        ...
        case CallTerm.Access access -> {
          ...
          if (access.of() instanceof IntroTerm.New n) {
            ...
            yield fieldBody.subst(buildSubst(fieldDef.ownerTele, access.structArgs())).normalize(state, NormalizeMode.NF);
          } else {
        ...
        case CallTerm.Hole hole -> {
          ...
          yield body.subst(buildSubst(def.fullTelescope(), hole.fullArgs())).normalize(state, NormalizeMode.NF);
        }
