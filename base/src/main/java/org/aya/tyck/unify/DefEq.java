...
public final class DefEq {
  ...
  private boolean compareLevel(int l, int r) {
    var lvalid = l != FormTerm.Univ.UNKNOWN_LIFT;
    var rvalid = r != FormTerm.Univ.UNKNOWN_LIFT;
    var valid = lvalid && rvalid;
    switch (cmp) {
      case Eq:
        if (valid && l != r) {
      ...
      case Gt:
        if (valid && l < r || lvalid && r == FormTerm.Univ.UNKNOWN_LIFT) {
          reporter.report(new LevelError(pos, l, r, false));
          return false;
        }
      case Lt:
        if (valid && l > r || rvalid && l == FormTerm.Univ.UNKNOWN_LIFT) {
