...
public interface VarCtx {
    @NotNull ImmutableSeq<FreeJavaExpr> toExprSeq(@NotNull FreeCodeBuilder cb, int limit);
    ...
    record SeqView(LocalVariable seq) implements VarCtx {
        ...
        public @NotNull ImmutableSeq<FreeJavaExpr> toExprSeq(@NotNull FreeCodeBuilder cb, int limit) {
            return AbstractExprializer.fromSeq(cb, Constants.CD_Term, seq.ref(), limit);
        }
    ...
    record SepVars(ImmutableSeq<AsmVariable> seq) implements VarCtx {
        @Override
        public @NotNull ImmutableSeq<FreeJavaExpr> toExprSeq(@NotNull FreeCodeBuilder cb, int limit) {
            return seq.take(limit).map(AsmVariable::ref);
        }
